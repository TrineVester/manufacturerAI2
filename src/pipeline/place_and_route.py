"""Routing coordinator with rotation-recovery fallback.

Takes an already-placed ``FullPlacement`` (produced by the placement stage),
runs routing, and — if routing fails — tries rotating each
``blocks_routing=True`` auto-placed component through its alternative
orientations (0/90/180/270°) to open routing corridors.

The caller receives a ``RouteResult`` that bundles the winning
``FullPlacement`` and ``RoutingResult``, plus a list of any rotation
changes applied, so the caller can persist both artifacts.
"""
...
@dataclass
class RouteResult:
    """Result from the routing coordinator."""
...
def route_with_recovery(
    ...
    *,
    ...
    """Route a placement, with automatic rotation-recovery on failure.

    Steps:
      1. Route the given placement via ``route_traces``.
      2. If routing is complete → return immediately.
      3. Otherwise: for each ``blocks_routing=True`` auto-placed component,
         try each of its three alternative 90° rotations (XY position fixed).
         Re-run routing only — no re-placement.
      4. Accept the first fully-routed variant; if none fully route, keep
         the variant with the fewest failed nets.
    """
